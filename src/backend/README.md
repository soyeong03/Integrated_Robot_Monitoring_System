...
FastAPI 기반의 백엔드 API 서버 및 InfluxDB 더미 데이터 생성기
...
├── main_with_influx.py        # InfluxDB 연동 API 서버 (추천)
├── generate_dummy_data.py     # 더미 데이터 생성기 (대화형)
├── quick_generate.py          # 더미 데이터 빠른 생성
├── requirements.txt           # Python 의존성
├── Dockerfile                 # Docker 이미지 빌드 설정
├── .env.example              # 환경 변수 예시
├── 빠른시작.md               # 5분 빠른 시작 가이드 (한국어)
├── USAGE_GUIDE.md            # 상세 사용 가이드 (한국어)
└── README_DUMMY_DATA.md      # 더미 데이터 생성기 상세 문서
```
...
- **[README_DUMMY_DATA.md](./README_DUMMY_DATA.md)** - 더미 데이터 생성기 상세 문서
...
# 더미 데이터 생성 (컨테이너에서)
docker-compose run --rm backend python quick_generate.py
...
uvicorn main_with_influx:app --reload --host 0.0.0.0 --port 8000
```
