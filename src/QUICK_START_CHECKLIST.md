...
**해결:**
1. 백엔드 터미널 확인
2. `cd backend && python main_with_influx.py` 실행
...
- **더미 데이터**: [backend/README_DUMMY_DATA.md](./backend/README_DUMMY_DATA.md)
- **백엔드 가이드**: [backend/USAGE_GUIDE.md](./backend/USAGE_GUIDE.md)
...
2. **InfluxDB 연동 (선택)**
   - Docker로 InfluxDB 시작: `docker-compose up -d influxdb`
   - 브라우저에서 http://localhost:8086 설정
   - 더미 데이터 생성: `cd backend && python quick_generate.py`
