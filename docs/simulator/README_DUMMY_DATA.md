...
```bash
nohup python simulator/generate_dummy_data.py > dummy_data.log 2>&1 &
```
...
- `/backend/fastapi/main.py`: FastAPI 백엔드 서버
- `/backend/fastapi/requirements.txt`: Python 패키지 목록
- `/.env`: 프로젝트 루트 환경 변수 설정
